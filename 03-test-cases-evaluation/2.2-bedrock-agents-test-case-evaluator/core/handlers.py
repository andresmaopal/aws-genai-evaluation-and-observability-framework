...
from .tracing import set_span_attributes, DateTimeEncoder
from typing import Dict, Any
    ...
    if "modelInvocationInput" in preprocessing_trace:
        ...
        if "inferenceConfiguration" in model_input:
            preprocessing_span.set_attribute(
                ...
                safe_json_dumps(model_input["inferenceConfiguration"]),
            )
    ...
    with tracer.start_as_current_span(
        ...
        with tracer.start_as_current_span(
            ...
            if "metadata" in model_output:
                ...
                assessment_span.set_attribute("metadata", safe_json_dumps(metadata))
            ...
            if "parsedResponse" in model_output:
                ...
                assessment_span.set_attribute(
                    "parsedResponse", safe_json_dumps(parsed_response)
                )
    ...
    if "modelInvocationOutput" in component_trace:
        ...
        with tracer.start_as_current_span(
            ...
            with tracer.start_as_current_span(
                ...
                if "metadata" in model_output:
                    metadata = model_output["metadata"]
                    output_span.set_attribute("metadata", safe_json_dumps(metadata))
...
def handle_knowledge_base(trace_data: Dict[str, Any], parent_span):
    ...
    if (
        ...
        set_span_attributes(
            kb_span,
            {
                "metadata": safe_json_dumps(kb_metadata),
                "kb.query.text": kb_input.get("text", ""),
                "kb.data_source": kb_input.get("dataSource", ""),
                "kb.filters": safe_json_dumps(kb_input.get("filters", {})),
            },
        )
    ...
    if (
        ...
        with tracer.start_as_current_span(
            ...
            if "retrievedReferences" in kb_output:
                ...
                kb_result_span.set_attribute(
                    SpanAttributes.LLM_COMPLETIONS, safe_json_dumps(results)
                )
    ...
    if (
        ...
        action_span = tracer.start_span(
            ...
            attributes={
                ...
                "tool.function": safe_json_dumps(action_input.get("function", {})),
                "trace.type": action_input.get("executionType", ""),
                SpanAttributes.LLM_REQUEST_MODEL: parent_span.attributes.get(
                    SpanAttributes.LLM_REQUEST_MODEL, "Not-Configured"
                ),
                "tool.parameters": safe_json_dumps(action_input.get("parameters", {})),
                SpanAttributes.SPAN_START_TIME: start_time,
                SpanAttributes.SPAN_END_TIME: end_time,
                SpanAttributes.SPAN_DURATION: duration,
            },
        ...
        set_span_attributes(
            action_span,
            {
                ...
                SpanAttributes.LLM_PROMPTS: safe_json_dumps(
    ...
    if (
        ...
        code_span = tracer.start_span(
            ...
            attributes={
                ...
                "gen_ai.tool_calls.0.arguments": safe_json_dumps(
        ...
        with tracer.start_as_current_span(
            ...
            if has_substantive_assessment:
                guardrail_span.set_attribute(
                    "guardrail.output_assessments", safe_json_dumps(combined_assessments)
                )
                    ...
                    for idx, assessment in enumerate(
                        ...
                        if "contentPolicy" in assessment:
                            assessment_span.set_attribute(
                                ...
                                safe_json_dumps(assessment["contentPolicy"]),
                            )

                        if "topicPolicy" in assessment:
                            assessment_span.set_attribute(
                                f"{prefix}topic_policy",
                                safe_json_dumps(assessment["topicPolicy"]),
                            )

                        if "wordPolicy" in assessment:
                            assessment_span.set_attribute(
                                f"{prefix}word_policy",
                                safe_json_dumps(assessment["wordPolicy"]),
                            )

                        if "sensitiveInformationPolicy" in assessment:
                            assessment_span.set_attribute(
                                f"{prefix}sensitive_info_policy",
                                safe_json_dumps(assessment["sensitiveInformationPolicy"]),
                            )
    ...
    if (
        ...
        with tracer.start_as_current_span(
            ...
            if "metadata" in final_response:
                final_response_span.set_attribute(
                    "response.metadata", safe_json_dumps(final_response["metadata"])
                )
...
def handle_guardrail_intervention(trace_data: Dict[str, Any], parent_span):
    ...
    with tracer.start_as_current_span(
        ...
        guardrail_span.set_attribute(
            SpanAttributes.LLM_COMPLETIONS, safe_json_dumps(assessments)
        )
        ...
        if blocked_items:
            guardrail_span.set_attribute(
                "guardrail.blocked_items", safe_json_dumps(blocked_items)
            )
...
def handle_guardrail_pre(trace_data: Dict[str, Any], parent_span):
    ...
    with tracer.start_as_current_span(
        ...
        guardrail_pre_span.set_attribute(
            SpanAttributes.LLM_COMPLETIONS, safe_json_dumps(input_assessments)
        )

        # Store assessments information
        guardrail_pre_span.set_attribute(
            "guardrail.assessments",
            safe_json_dumps({"input": guardrail_trace.get("inputAssessments", [])}),
        )
...
def handle_guardrail_post(trace_data: Dict[str, Any], parent_span):
    ...
    with tracer.start_as_current_span(
        ...
        if output_assessments:
            guardrail_span.set_attribute(
                "guardrail.output_assessments", safe_json_dumps(output_assessments)
            )
            ...
            with tracer.start_as_current_span(
                ...
                if output_assessments and len(output_assessments) > 0:
                    ...
                    if "contentPolicy" in assessment:
                        assessment_span.set_attribute(
                            "content_policy", safe_json_dumps(assessment["contentPolicy"])
                        )
                    if "topicPolicy" in assessment:
                        assessment_span.set_attribute(
                            "topic_policy", safe_json_dumps(assessment["topicPolicy"])
                        )
                    if "wordPolicy" in assessment:
                        assessment_span.set_attribute(
                            "word_policy", safe_json_dumps(assessment["wordPolicy"])
                        )
                    if "sensitiveInformationPolicy" in assessment:
                        assessment_span.set_attribute(
...
def handle_user_input_span(trace_data: Dict[str, Any], parent_span):
    ...
    with tracer.start_as_current_span(
        ...
        attributes={
            ...
            "tool.parameters": safe_json_dumps({
                "question": {
                    "type": "string",
        ...
        if obs:
            ...
            if "ask_user_metadata" in obs:
                user_input_span.set_attribute(
                    "ask_user.metadata", safe_json_dumps(obs["ask_user_metadata"])
                )
...
def handle_file_operations(trace_data: Dict[str, Any], parent_span):
    ...
    with tracer.start_as_current_span(
        ...
        attributes={
            ...
            "file.types": safe_json_dumps([f.get("type", "unknown") for f in files_list]),
            SpanAttributes.SPAN_START_TIME: start_time,
            SpanAttributes.SPAN_END_TIME: end_time,
            SpanAttributes.SPAN_DURATION: duration,
        },
        ...
        for idx, this_file in enumerate(files_list):
            ...
            if "metadata" in this_file:
                file_span.set_attribute(
                    f"file.{idx}.metadata",
                    safe_json_dumps(this_file["metadata"])
                )
